...
function App() {
  ...
  async function inspectProfilePhoto(dataUrl) {
    ...
    const box = faceBox(face);
    const size = Math.min(box.width / image.naturalWidth, box.height / image.naturalHeight);
    const probability = Number(face.probability?.[0] ?? face.probability ?? 1);
    return size >= 0.12 && probability >= 0.7;
  }
    ...
    {screen === 'lobby' ? <section className="lobby">
      ...
      {sheet==='shop'&&<><h2 id="sheet-title">내 포인트</h2><div className="point-balance"><Coins size={24}/><strong>{wallet.balance.toLocaleString()}<small> P</small></strong></div>
        ...
        <div className="shop-list">
          <div className="shop-row"><span><strong>가입 축하</strong><small>첫 가입 2,000P · 재가입은 지급되지 않아요</small></span><b>+2,000P</b></div>
        ...
        <h3 ref={subscriptionSection} className="section-title">정기 구독</h3><div className={`subscription-card ${subscriptionCancelAt?'is-canceling':''}`}><div><strong>{SUBSCRIPTIONS[0].name}</strong><small>{subscriptionCancelAt?`${subscriptionEndLabel()}까지 이용할 수 있어요.`:'입장·연장·미리보기 포인트 없이'}</small></div><div className="subscription-cta"><b>{subscription?subscriptionCancelAt?'해지 예약됨':'이용 중':`${SUBSCRIPTIONS[0].price.toLocaleString()}원`}</b><Button size="small" variant="weak" onClick={requestSubscriptionChange}>{subscription?(subscriptionCancelAt?'해지 취소':'해지하기'):'시작하기'}</Button></div></div><p className="shop-note">매월 자동 결제돼요. 해지는 토스 결제 내역에서 하고, 기간이 끝날 때까지 쓸 수 있어요. 자리 양보는 빠져요.</p><h3 className="section-title">이렇게 사용해요</h3><div className="shop-list point-uses"><span>입장<strong>500P</strong></span><span>시간 연장<strong>30분 500P</strong></span><span>손님 미리보기<strong>500P</strong></span><span>자리 양보 부탁<strong>수락 시 500P</strong></span></div>
