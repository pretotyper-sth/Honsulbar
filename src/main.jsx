...
function App() {
  ...
  useEffect(() => {
    if (sheet !== 'attendance-ad') return;
    setAdSeconds(0);
    const timer = setInterval(() => setAdSeconds(value => Math.min(5, value + 1)), 1000);
  ...
  function attendance() {
    ...
    setSheet('attendance-ad');
  }
  function claimAttendanceAfterAd() {
    const day=new Intl.DateTimeFormat('sv-SE',{timeZone:'Asia/Seoul'}).format(new Date());
    if (adSeconds < 5 || latest.current.wallet.attendanceDate===day) return;
  ...
  function choosePhoto(event) {
    ...
    setProfile(v => ({ ...v, photo: URL.createObjectURL(file) })); setProfileVerified(false); setError('');
  }
  ...
  async function captureVerification() {
    ...
    try {
      ...
      const cameraFaces = await model.estimateFaces(video, false);
  ...
  return <main className="app">
    ...
    {screen === 'lobby' ? <section className="lobby">
      ...
      {sheet==='shop'&&<><h2 id="sheet-title">내 포인트</h2><div className="point-balance"><Coins size={24}/><strong>{wallet.balance.toLocaleString()}<small> P</small></strong></div>
        <h3 className="section-title">포인트 모으기</h3><div className="earn-row"><span><strong>매일 출석</strong><small>광고 시청 후 하루 한 번 1,000P</small></span><Button size="small" variant="weak" disabled={wallet.attendanceDate===today} onClick={attendance}>{wallet.attendanceDate===today?'오늘 받았어요':'광고 보고 받기'}</Button></div>
      ...
      {sheet==='attendance-ad'&&<><p className="eyebrow">매일 출석</p><h2 id="sheet-title">광고를 보고 포인트를 받아요</h2><p className="sheet-description">광고를 끝까지 본 뒤 오늘의 출석 포인트 1,000P를 받을 수 있어요.</p><div className="ad-preview" aria-label="광고 시청 영역"><Eye size={24}/><strong>{adSeconds < 5 ? `광고 시청 중 ${adSeconds}/5초` : '광고 시청 완료'}</strong><small>광고 시청이 끝나면 포인트를 받을 수 있어요.</small></div><Button size="xlarge" display="block" disabled={adSeconds < 5} onClick={claimAttendanceAfterAd}>{adSeconds < 5 ? '광고 시청 중…' : '1,000P 받기'}</Button></>}
